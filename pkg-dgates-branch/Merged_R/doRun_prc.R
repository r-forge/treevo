	...
	if(nStepsPRC>1){
		for (step in 2:nStepsPRC) {
			...
			while (particle<=numParticles) { 
				...
				for(newparticleList in listpartvec){
				attempts<-attempts+1
			...
			boxcoxOneSimSumStats<-boxcoxTransformation(summaryStatsLong(phy, convertTaxonFrameToGeigerData(doSimulation(splits, intrinsicFn, extrinsicFn, newparticleList$startingValues, newparticleList$intrinsicValues, newparticleList$extrinsicValues, timeStep), phy)), boxcoxAddition, boxcoxLambda)#this could be quite inefficient but it's needed for the next steps...
...
#####Dans new startFromCheckpoint#####

#for now just have user specify the workspace file (perhaps want to add something after everystep like "checkpoint saved in PATH"

#This also assumes you've completed at least the initial simulations
#This currently works for a run saved after the first prc step

load(checkpointFile)

if (dataGenerationStep ==0 ){ #only completed simulations
...
#while(sink.number()>0) {sink()}
	...
	if(nStepsPRC>1){
		for (step in 2:nStepsPRC) {
			...
			while (particle<=numParticles) { #skipping this for now...
				#attempts<-attempts+1
				particleFn<-function(){
				newparticleList<-list(abcparticle(id=particle, generation=1, weight=0))
				newparticleList[[1]]<-initializeStatesFromMatrices(newparticleList[[1]], startingPriorsValues, startingPriorsFns, intrinsicPriorsValues, intrinsicPriorsFns, extrinsicPriorsValues, extrinsicPriorsFns)
...
if (dataGenerationStep > 0){ #has completeted the first run
